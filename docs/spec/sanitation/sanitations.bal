...
public function main(string apiName = "API_SALES_ORDER_SRV") returns error? {
    check sanitizeSchemaNames(apiName);
    check sanitizeEnumParamters(apiName);
}

function sanitizeEnumParamters(string apiName) returns error? {
...
function sanitizeSchemaNames(string apiName) returns error? {
    ...
    check io:fileWriteString(string `../${apiName}/${apiName}.json`, updatedSpec);
}
